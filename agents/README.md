...
## 📦 Directory Overview — `/suite/` (Full Suite Overview)
...
### **Screening A[i]gent**
Rubric-driven, hybrid-scoring system for consistent, explainable resume evaluation.  
Surfaces hidden talent, reduces noise, and provides structured signals to downstream agents.
...
### **Workflow A[i]gent**
Interview-stage automation across confirmations, reminders, scorecard SLAs, and delay updates — creating predictable, low-drama interview cycles.
...
- **Metrics Dictionary:** Google Sheet (linked in playbook)
...
1. Start with the **Suite Overview** (`/agents/suite/`).  
2. Explore each agent’s **live demo** (`index.html`).  
3. Read each **playbook** for operational and governance detail.  
4. Reference the **Metrics Dictionary** for cross-agent data definitions.  
